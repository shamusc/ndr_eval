...
# methods = ['UMAP','MDS','Isomap','PCA']
# col_labels= ["Embedded Dimension"]+methods
col_labels = ['Random Embedding']
results = []
dim = 5
while dim < 500:
    new_row=[dim]
    # embed the data in lower dimension:
    print("Running " +str(dim)+" Dimensions...")
    random_embedding = hypersphere(n_dimensions=dim,n_samples=1778,k_space=dim)
    # create a minimum spanning tree in low d:
    print("Generating low-D tree...")
    lowD_tree = mst(random_embedding)
...
frame_of_data= pd.DataFrame(nparray)
frame_of_data.to_csv("data/ged_random_embedding.csv")
