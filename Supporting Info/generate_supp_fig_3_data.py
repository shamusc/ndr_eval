...
n_samples = 1000
cluster_sizes = range(1,150,1)
dim_sizes = range(1,21,1)
list_array =[cluster_sizes]
case_runs = range(0,5,1)
#data = hypersphere(n_dimensions=4,n_samples=n_samples,k_space=20)
for case_run in case_runs:
    run_array = []
    data = hypersphere(n_dimensions=5,n_samples=n_samples,k_space=20)
...
col_labels= ['# of Nearest Neighbors']+["Trial: "+ str(p+1) for p in case_runs]
print("Making DataFrame");
frame_of_data= pd.DataFrame(nparray, columns=col_labels)
print("Making.csv")
frame_of_data.to_csv("/home/shamuscooley/Research/manifold_learning/figures/supplement/normal_dist/data/supp3-2.csv")
print("All Done")
